        ...
        local function createAnimation(self, v1, v2, duration, mode, typ, get, set)
            ...
            activeAnimations[typ] = {}
                activeAnimations[typ].call = function()
                    ...
                    local _v1 = math.floor(lerp.lerp(v1Val, v1, lerp[mode](progress+0.05 / duration))+0.5)
                    local _v2 = math.floor(lerp.lerp(v2Val, v2, lerp[mode](progress+0.05 / duration))+0.5)
                    set(self, _v1, _v2)
                end
            ...
            eventHandler = function(self, event, timerId, ...)
                ...
                if(event=="timer")then
                    ...
                    if(animation~=nil)then
                        if(animation.progress+0.05<animation.duration)then
                            animation.call()
